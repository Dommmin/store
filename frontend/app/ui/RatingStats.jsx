...
const RatingStats = ({ ratings, averageRating = 0, ratingsCount = 0, className = '' }) => {
    ...
    return (
        <div className={className}>
            <div
                className="w-full grid grid-cols-2 p-4 mx-auto bg-dark-500 rounded-xl flex items-center space-x-4 text-white">
                    ...
                    {ratings.map((rating, i) => (
                        <div key={i} className="flex items-center space-x-2">
                            ...
                            <div className="flex justify-start"
                                 style={{width: '30px', textAlign: 'right'}}>{rating}</div>
                        </div>
                    ))}
                </div>
            </div>
